...
```{r}
ggplot() +
  geom_density(data = complete(MasterImp, 1), aes(x = PCSSF_TotalScore_Preop, color = "Observed")) +
  geom_density(data = complete(MasterImp, 2:10), aes(x = PCSSF_TotalScore_Preop, color = "Imputed")) +
  labs(title = "PCS-SF: Observed vs. Imputed Distributions")
```
...
PCSMod <- with(
  MasterImp, 
  exp = lm(
    PCSSF_TotalScore_Preop ~ Region + AgeAtInitialExam + Sex2 + BilateralPres + SRCQTotalScore + VR12_Physical_TotalScore_Preop + VR12_Mental_TotalScore_Preop
    )
)
...
Table 2: Summary of patient and treatment record characteristics for the analysed sample.
...
```{r}
PCSprPlot1 <- ggplot(data = MasterAnalysis, mapping = aes(y = Region, x = PCSSF_TotalScore_Preop)) + stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7) +
  stat_dotsinterval(side = "bottom", scale = 0.7, slab_linewidth = NA) +
  scale_fill_brewer(palette = "Set2")

knitr::knit_print(PCSprPlot1)
```
...
```{r}
PCSfit <- gtsummary::tbl_regression(
  PCSMod,
...
knitr::knit_print(PCSfit)
```

```{epoxy}

Analysis of pain catastrophizing scores revealed significant differences across anatomical regions (Table 5), after adjustment for demographic characteristics and patient-reported health measures. Compared to ankle conditions (reference group), patients with forefoot (β = {gtsummary::inline_text(PCSfit, variable = "Region", level = "Forefoot")}) and rearfoot (β = {gtsummary::inline_text(PCSfit, variable = "Region", level = "Rearfoot")}) and multiple region involvement (β = {gtsummary::inline_text(PCSfit, variable = "Region", level = "Multiple")}) demonstrated higher pain catastrophizing scores at baseline. The model adjusted for age, sex, bilateral presentation status, comorbidity burden, and physical and mental health component scores. This suggests that anatomical location of the condition may be independently associated with baseline pain catastrophizing, with rearfoot conditions showing the strongest association.
...
PCSPredict <- marginaleffects::predictions(
  PCSMod,
  type = "response"
  ) |> dplyr::mutate(
  PCSCat = if_else(
    PCSSF_TotalScore_Preop >= 7, "Yes","No"
  )
)
...
```{r}
PCSprPlot2 <- ggplot(data = PCSPredict, mapping = aes(y = Region, x = PCSSF_TotalScore_Preop)) + stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7) +
  stat_dotsinterval(side = "bottom", scale = 0.7, slab_linewidth = NA) +
  scale_fill_brewer(palette = "Set2") +
  ggtitle('PCS Predicted by Region')

knitr::knit_print(PCSprPlot2)
```

Figure 8: Model predicted PCS-SF compared between diagnostic groups - region labels
